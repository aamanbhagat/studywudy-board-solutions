  ...
  return legalPage({
    ...
    body: `<p class="phase5-muted">Methodology last updated: 23 August 2026</p>
      ...
      <div class="phase5-policy-grid">
        ...
        <section><h2>Numerical</h2><p>Checks source-input integrity first, then the formula, substitution, units, arithmetic and final answer, as well as readable equations.</p></section>
      ...
      <h2>Source-input integrity</h2>
      <p>The source-input gate runs before answer-quality and equation checks. It compares the imported prompt with the normalized public prompt, the worked solution's Given values, formula substitutions and final answer. Unrecorded numeric changes fail closed. Recorded corrections remain under <code>noindex</code> until their source-review disposition explicitly approves publication.</p>
      <p>The scanner quarantines ambiguous OCR such as <code>loo</code>, <code>l00</code> and <code>I00</code> in numeric contexts, placeholder text attached to quantities, letter O or I inside otherwise numeric values, malformed decimal separators and broken unit boundaries. It also compares near-duplicate numeric prompts for a suspicious added or dropped digit and checks that questions asking for a count of discrete objects do not end in an unexplained fractional result.</p>
