...
function auditCorpus() {
  ...
  try {
    ...
    return Object.freeze({ report, duplicateRowIds, duplicateRows });
  } finally {
    database.close();
  }
}
...
function manifestSource(result) {
  const runtimeDuplicateRowIds = result.duplicateRowIds.filter((rowId) => isQuestionRowIndexable(PHASE4_GATE_MANIFEST, rowId));
  const metadata = {
    policyVersion: CORPUS_QUALITY_POLICY_VERSION,
